...
import { ThumbsUp, MessageCircle, CheckCircle, Plus, Users, TrendingUp, Info } from 'lucide-react';
...
export default function EqualDemocracyApp() {
  ...
  const handleFinalVote = (proposalId, choice) => {
    const vote = new Vote(Date.now().toString(), proposalId, currentUser.id, choice);
  ...
  const hasVoted = (proposalId) => {
    return finalVotes.some(v => v.proposalId === proposalId && v.userId === currentUser.id);
  };

  // Render views based on current state
  if (view === 'welcome' && !currentUser) {
    return <WelcomeView onCreateUser={handleCreateUser} onAbout={() => setView('about')} />;
  }

  if (view === 'about') {
    return <AboutView onBack={() => setView(currentUser ? 'home' : 'welcome')} />;
  }
  ...
  if (view === 'home') {
    return (
      <HomeView
        ...
        onDiscuss={(p) => { setSelectedProposal(p); setView('discuss'); }}
  ...
  if (view === 'create') {
    return <CreateProposalView onSubmit={handleCreateProposal} onBack={() => setView('home')} />;
  }
  ...
  return <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">Laddar...</div>;
}
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-blue-800 flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-lg text-gray-600">Demokrati i din stad</p>
        </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 space-y-6">
          <div className="text-center">
            ...
            <h1 className="text-3xl font-bold text-blue-800 mb-2">Equal Democracy</h1>
            <p className="text-gray-600">Version 1.0 - MVP</p>
          </div>

          <div className="space-y-4 text-gray-700">
            <h2 className="text-xl font-semibold text-blue-800">Vad är Equal Democracy?</h2>
            <p>
              Equal Democracy är en app för demokratiskt beslutsfattande som ger alla medborgare möjlighet 
              att delta i viktiga beslut som påverkar deras stad och samhälle.
            </p>

            <h3 className="text-lg font-semibold text-blue-800 mt-6">Hur fungerar det?</h3>
            <ol className="list-decimal list-inside space-y-2 ml-2">
              <li><strong>Föreslå idéer</strong> - Dela dina förslag för att förbättra staden</li>
              <li><strong>Rösta med tummen upp</strong> - Stöd de idéer du gillar</li>
              <li><strong>Diskutera topp 3</strong> - Förfina de mest populära förslagen tillsammans</li>
              <li><strong>Rösta slutgiltigt</strong> - Bestäm vilka idéer som ska förverkligas</li>
            </ol>

            <h3 className="text-lg font-semibold text-blue-800 mt-6">Designprinciper</h3>
            <p>
              Appen är designad med svenska värderingar i åtanke - enkel, inkluderande och svår att 
              misslyckas med. Inspirerad av IKEA:s demokratiska design.
            </p>

            <h3 className="text-lg font-semibold text-blue-800 mt-6">Framtidsvisionen</h3>
            <p>
              Detta är en MVP (Minimal Viable Product) för att testa konceptet lokalt inför valet i 
              september 2026. Om Equal Democracy visar sig användbart kan systemet skalas upp med 
              fler moduler och till slut kanske bli en del av en global demokratisk infrastruktur - 
              ett "Global Citizen Council" där alla världens medborgare kan delta i viktiga beslut.
            </p>

            <div className="bg-blue-50 rounded-xl p-4 mt-6">
              <p className="text-sm text-blue-800">
                <strong>Offline-först:</strong> Appen fungerar utan internetuppkoppling och synkar 
...
function HomeView({ proposals, currentUser, onCreateProposal, onThumbsUp, onDiscuss, onMoveToTop3, onVote, onAbout }) {
  const activeProposals = proposals.filter(p => p.status === 'active').sort((a, b) => b.thumbsUp - a.thumbsUp);
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">Equal Democracy</h1>
                <p className="text-blue-100 text-sm">Hej, {currentUser.name}!</p>
              </div>
            </div>
            <button onClick={onAbout} className="text-white hover:text-yellow-400">
              <Info className="w-6 h-6" />
            </button>
          </div>
          <h2 className="text-xl font-medium">Hur vill du förbättra vår stad?</h2>
        </div>
      </div>
          ...
          <div className="bg-yellow-50 border-2 border-yellow-400 rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-bold text-blue-800">Topp 3 förslag</h3>
              </div>
              <button
                onClick={onVote}
            ...
            <p className="text-gray-700">Diskutera och rösta på de mest populära idéerna</p>
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          {activeProposals.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 text-center text-gray-500">
              <p>Inga förslag än. Var den första att föreslå något!</p>
            </div>
          ) : (
    ...
    <div className="bg-white rounded-2xl shadow-md p-6 space-y-4 hover:shadow-lg transition-shadow">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <h4 className="text-lg font-bold text-blue-800 mb-2">{proposal.title}</h4>
          <p className="text-gray-600">{proposal.description}</p>
          <p className="text-sm text-gray-400 mt-2">Av {proposal.authorName}</p>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-blue-800 mb-6">Föreslå en ny idé</h2>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-blue-600 text-white p-6 shadow-lg">
        <div className="max-w-2xl mx-auto">
          <button onClick={onBack} className="mb-4 text-white hover:text-yellow-400 font-medium">
            ← Tillbaka
          </button>
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          {proposal.comments.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 text-center text-gray-500">
              <p>Inga kommentarer än. Var den första att diskutera!</p>
            </div>
          ) : (
            proposal.comments.map(comment => (
              <div key={comment.id} className="bg-white rounded-2xl shadow-md p-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-gray-800">{comment.authorName}</p>
...
function VoteView({ proposals, currentUser, onVote, hasVoted, getVoteResults, onBack }) {
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-green-600 text-white p-6 shadow-lg">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-green-100 mt-2">Välj vilka idéer som ska förverkligas</p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6">
        {proposals.map((proposal, index) => {
          ...
          return (
            <div key={proposal.id} className="bg-white rounded-2xl shadow-lg p-6 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 bg-green-100 rounded-lg p-3 text-center">
                      <p className="text-2xl font-bold text-green-700">{results.yes}</p>
                      <p className="text-xs text-green-600">Ja</p>
                    </div>
                    <div className="flex-1 bg-red-100 rounded-lg p-3 text-center">
                      <p className="text-2xl font-bold text-red-700">{results.no}</p>
                      <p className="text-xs text-red-600">Nej</p>
                    </div>
                  </div>
                  <p className="text-center text-sm text-green-600 font-medium">✓ Du har röstat</p>
                </div>
              )}
            </div>
          );
