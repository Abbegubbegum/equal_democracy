...
function TaxIncomeSliders({
  ...
  const minTaxRate =
    taxRateInfo?.minTaxRateKr || Math.floor(defaultTaxRate * 0.9 * 100) / 100;
  const maxTaxRate =
    taxRateInfo?.maxTaxRateKr || Math.ceil(defaultTaxRate * 1.1 * 100) / 100;

  // State: we store the tax rate (kr), and calculate amount from it
  const [taxRateKr, setTaxRateKr] = useState(
    allocation?.taxRateKr || defaultTaxRate,
  );
  ...
  const defaultTaxRatePercentage =
    ((defaultTaxRate - minTaxRate) / (maxTaxRate - minTaxRate)) * 100;
  const currentTaxRatePercentage =
    ((taxRateKr - minTaxRate) / (maxTaxRate - minTaxRate)) * 100;

  const minAmount = minTaxRate * taxBase;
  const maxAmount = maxTaxRate * taxBase;
  const defaultAmount = defaultTaxRate * taxBase;
  const defaultAmountPercentage =
    ((defaultAmount - minAmount) / (maxAmount - minAmount)) * 100;
  const currentAmountPercentage =
    ((calculatedAmount - minAmount) / (maxAmount - minAmount)) * 100;
 ...
 */
export default function IncomeCategoryInput({
  category,
  allocation,
  onUpdate,
  taxRateInfo,
  ...
  const isTaxIncome =
    category.isTaxRate || category.name.toLowerCase().includes("skatt");

  // Calculate taxBase from existing data if not provided
  // Formula: taxBase = Skatteintäkter / Skattesats (kr)
  const calculatedTaxBase =
    !taxRateInfo && isTaxIncome && category.amount > 0
  ...
  const minValue =
    category.minAmount && category.minAmount < defaultValue
      ? category.minAmount
