 ...
 */
export default function LayeredTreemaps({
  expenseCategories,
  incomeCategories,
  onExpenseChange: _onExpenseChange,
  onIncomeChange: _onIncomeChange,
  taxBaseInfo,
  onViewModeChange,
  onCategoryClick,
}) {
  ...
  const totalFullExpenses =
    expenseCategories?.reduce((sum, cat) => sum + (cat.amount || 0), 0) || 0;
  const totalFullIncome =
    incomeCategories?.reduce((sum, cat) => sum + (cat.amount || 0), 0) || 0;
  ...
  const expenseWidthPercent =
    maxTotal > 0
      ? Math.max(
          20,
          Math.min(
            100,
            (totalFullExpenses / maxTotal) * 100 -
              (totalFullExpenses < totalFullIncome
                ? (amplifiedDifference / maxTotal) * 100
                : 0),
          ),
        )
      : 100;
  const incomeWidthPercent =
    maxTotal > 0
      ? Math.max(
          20,
          Math.min(
            100,
            (totalFullIncome / maxTotal) * 100 -
              (totalFullIncome < totalFullExpenses
                ? (amplifiedDifference / maxTotal) * 100
                : 0),
          ),
        )
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-visible">
      ...
      >
        ...
        >
          <SimpleTreemap
            categories={scaledIncomeCategories}
            taxBaseInfo={taxBaseInfo}
