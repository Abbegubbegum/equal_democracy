...
export default function SurveyPage() {
  ...
  const fetchSurvey = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/survey?visitorId=${encodeURIComponent(visitorId)}`,
      );
  ...
  const totalVotes =
    survey?.choices?.reduce((sum, c) => sum + (c.voteCount || 0), 0) || 0;
        ...
        <div className="max-w-2xl mx-auto p-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-2">
              {t("poll.noActivePoll") || "No Active Poll"}
            </h1>
            <p className="text-gray-600">
              {t("poll.checkBackLater") ||
                  ...
                  <div className="relative flex items-center justify-between">
                    ...
                    {showResults && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        {totalVotes > 0 && (
                          <span
                            className={`font-medium ${isSelected ? "text-primary-700" : "text-gray-600"}`}
                          >
                            ({percentage}%)
                          </span>
                        )}
