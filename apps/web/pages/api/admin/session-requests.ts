...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    try {
      const request =
        await SessionRequest.findById(requestId).populate("userId");
      ...
      if (action === "approve") {
        const sessionsToGrant = grantedSessions || request.requestedSessions;
        ...
        try {
          await sendSessionRequestDenialNotification(user.email, user.name);
        } catch (emailError) {
      ...
      log.error("Failed to process session request", {
