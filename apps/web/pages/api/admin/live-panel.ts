...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      ...
      if (activeSession.phase === "phase2") {
        ...
        const votedPercentage =
          activeUsersCount > 0
            ...
            : 0;
        const terminationConditionMet =
          votedPercentage >= 75 && activeUsersCount > 0;
      ...
      const calculatedTopCount =
        totalProposals >= 2
          ? Math.max(
              2,
              Math.min(
                totalProposals,
                Math.round(1.2 * Math.sqrt(totalProposals)),
              ),
            )
          : 0;

      const proposalsPercentage =
        totalProposals > 0 ? (ratedProposals / totalProposals) * 100 : 0;
      const usersPercentage =
        activeUsersCount > 0
          ? (usersWhoRatedCount / activeUsersCount) * 100
          : 0;
      const conditionsMet =
        proposalsPercentage >= 75 &&
        usersPercentage >= 75 &&
        activeUsersCount > 0 &&
        totalProposals >= 2;
  ...
  if (req.method === "PATCH") {
    ...
    try {
      ...
      if (!activeSession) {
        return res
          .status(404)
          .json({ error: "Active phase1 session not found" });
      }
      ...
      return res
        .status(500)
        .json({ error: "Failed to update custom top count" });
    }
  }
