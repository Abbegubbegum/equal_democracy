  ...
  if (req.method === "GET") {
    try {
      const pendingApplications = await User.find({
        ...
        .select(
          "name email appliedForAdminAt organization requestedSessions createdAt",
        )
    ...
    try {
      ...
      try {
        if (action === "approve") {
          ...
          await sendAdminDenialNotification(user.email, user.name);
        }
      } catch (emailError) {
      ...
      log.error("Failed to process admin application", {
