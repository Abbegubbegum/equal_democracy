...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      ...
      const data = await Comment.find(filter).sort({ createdAt: -1 }).lean();

      // Collect proposal ids and build title map
      const proposalIds = [
        ...new Set(data.map((c) => c.proposalId?.toString?.()).filter(Boolean)),
      ];
    ...
    if (req.method === "DELETE") {
      ...
      await Comment.findByIdAndDelete(toObjectId(String(id)));
