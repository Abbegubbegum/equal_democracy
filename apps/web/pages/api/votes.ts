  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      if (rawSession?.tiebreakerActive) {
        const tiedIds = (rawSession.tiebreakerProposals || []).map((id) =>
          id.toString(),
        );
        if (!tiedIds.includes(proposalId.toString())) {
          return res.status(400).json({
            message:
              "Under förlängd omröstning kan du bara rösta på de oavgjorda förslagen.",
          });
        }
      }
      ...
      if (existingVoteInSession) {
        if (terminationActive) {
          ...
          return res.status(400).json({
            message: "You have already used your vote this session.",
          });
        }
      }
      ...
      if (shouldClose) {
        log.info("Session auto-closed after vote", {
  ...
  if (req.method === "GET") {
    ...
    if (proposalId) {
      ...
      try {
        ...
        const voteSessionId = activeSession
          ? activeSession._id
          : toObjectId(String(sessionId));
        const yesCount = await FinalVote.countDocuments({
  ...
  try {
    ...
    if (activeUserIds.length > 0) {
      ...
      if (allUsersVoted) {
        ...
        const result = (await closeSession(activeSession, {
          sendEmails: true,
        })) as any;
