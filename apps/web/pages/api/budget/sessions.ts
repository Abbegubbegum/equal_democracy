...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      ...
      const sessions = await BudgetSession.find(query).sort({ createdAt: -1 });
  ...
  if (req.method === "PUT") {
    ...
    try {
      ...
      log.error("Failed to update budget session", {
  ...
  if (req.method === "DELETE") {
    ...
    try {
      ...
      log.error("Failed to delete budget session", {
