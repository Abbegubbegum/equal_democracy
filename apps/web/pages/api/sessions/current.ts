...
export default async function handler(
  ...
  try {
    ...
    const rawSession = (await Session.findById(
      activeSession._id,
    ).lean()) as any;
    ...
    if (rawSession?.phase2TerminationScheduled) {
      ...
      terminationSecondsRemaining = Math.max(
        0,
        Math.floor((scheduledTime.getTime() - new Date().getTime()) / 1000),
      );
    }
    ...
    if (tiebreakerActive && rawSession?.tiebreakerScheduled) {
      ...
      tiebreakerSecondsRemaining = Math.max(
        0,
        Math.floor((tbTime.getTime() - new Date().getTime()) / 1000),
      );
      tiebreakerProposalIds = (rawSession.tiebreakerProposals || []).map((id) =>
        id.toString(),
      );
    }
    ...
    return res.status(200).json({
      ...
      showUserCount:
        activeSession.showUserCount !== undefined
          ? activeSession.showUserCount
          : false,
      noMotivation:
        activeSession.noMotivation !== undefined
