  ...
  const handleSave = async () => {
    ...
    if (isSuperAdmin) {
      ...
      if (isNaN(hours) || hours < 1 || hours > 168) {
        setMessage("Error: Session limit must be between 1 and 168 hours");
        return;
      }
    }
      ...
      <div className="space-y-4">
        ...
        {isSuperAdmin && (
          <div>
            ...
            />
            <p className="text-sm text-slate-500 mt-1">
              Sessions automatically close when everyone has voted or after this
              time limit (1-168 hours from session start)
            </p>
          </div>
        )}
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({
                              ...
                              isAdmin: e.target.checked ? true : f.isAdmin,
                            }))
                          }
                        />
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <span className="text-green-700">👍 {tp.yesVotes} yes</span>
                    <span className="text-red-700">👎 {tp.noVotes} no</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  const loadSessions = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setSessions(
          Array.isArray(data) ? data.filter((s) => s.status === "closed") : [],
        );
      } else {
    ...
    <section className="bg-white rounded-xl p-6 shadow space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600 mb-4">
          Send an email to all participants in a closed session with information
          about which proposals won.
        </p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Applied:{" "}
            {new Date(application.appliedForAdminAt).toLocaleDateString()}
          </p>
        </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  value={sessionLimit}
                  onChange={(e) =>
                    setSessionLimit(parseInt(e.target.value) || 10)
                  }
          ...
          {requests.map((request) => (
            <SessionRequestCard
              ...
              onApprove={(grantedSessions) =>
                handleRequest(request._id, "approve", grantedSessions)
              }
    ...
    <div className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-700 mt-1">
            <span className="font-medium">Current remaining sessions:</span>{" "}
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setGrantedSessions(parseInt(e.target.value) || 1)
                  }
                  className="w-20 border border-slate-300 rounded px-2 py-1 text-sm"
                />
              </div>
...
function CleanPanel() {
  ...
  const runClean = async () => {
    ...
    setErrorMsg("");
    try {
      const res = await fetchWithCsrf("/api/admin/clean-content", {
        method: "POST",
      });
      if (!res.ok) {
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            XAI granskar alla kommentarer och förslag och raderar olämpligt
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 space-y-1">
        ...
        <p>
          Raderat innehåll kan inte återställas. Endast innehåll som bryter mot
          lagen (svordomar, obscenitet, hat, hot) raderas — hetsiga politiska
          argument lämnas kvar.
        </p>
      </div>
      ...
      {status === "confirming" && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-5 space-y-4">
          <p className="font-semibold text-red-800">
            Är du säker? XAI kommer att skanna och permanent radera allt
            olämpligt innehåll.
          </p>
      ...
      {status === "running" && (
        <div className="flex items-center gap-3 text-slate-600 py-4">
          ...
          <span className="font-medium">
            XAI granskar allt innehåll… Det kan ta en stund.
          </span>
        </div>
      )}
      ...
      {status === "done" && result && (
        <div className="space-y-4">
          <div className="flex gap-6">
            ...
            <div
              className={`rounded-xl border p-4 text-center flex-1 ${result.removed > 0 ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}
            >
              <p
                className={`text-3xl font-bold ${result.removed > 0 ? "text-red-700" : "text-green-700"}`}
              >
                {result.removed}
              </p>
              <p
                className={`text-sm mt-1 ${result.removed > 0 ? "text-red-500" : "text-green-600"}`}
              >
                Raderade
              </p>
            </div>
          ...
          {result.removed === 0 && (
            <p className="text-green-700 font-medium">
              ✅ Allt innehåll är rent. Inga regelbrott hittades.
            </p>
          )}
            ...
            >
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex items-start gap-2 flex-wrap">
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-semibold ${STATUS_COLOR[p.status]}`}
                  >
