...
export default function BudgetVotingPage() {
  ...
  const initializeAllocations = useCallback(
    (budgetSessionData) => {
      ...
      if (incomeAllocations.length === 0) {
        const defaultIncomeAllocations = budgetSessionData.incomeCategories.map(
          (cat) => {
  ...
  const updateAllocation = useCallback((categoryId, newAmount) => {
    setAllocations((prev) => {
      ...
      if (existing) {
        ...
        return [...prev, { categoryId, amount: newAmount, subcategories: [] }];
      }
    });
  }, []);

  const updateIncomeAllocation = useCallback(
    (categoryId, newAmount, taxRateKr) => {
  ...
  const handleExpensePinch = useCallback(
  ...
  const handleIncomePinch = useCallback(
    (categoryId, newAmount) => {
  ...
  const handleSaveVote = useCallback(async () => {
    ...
    try {
      ...
      const totalIncome = incomeAllocations.reduce(
        (sum, a) => sum + a.amount,
        0,
      );
  ...
  const updatedIncomeCategories = budgetSession?.incomeCategories.map(
    (category) => {
      const allocation = incomeAllocations.find(
  ...
  const isSkolpeng = (name) =>
    name &&
    (name.toLowerCase().includes("skolpeng") ||
      name.toLowerCase().includes("gymnasieskolpeng"));
  const sortedExpenseCategories = budgetSession?.categories
    ? [...budgetSession.categories]
        .map((cat) => {
  ...
  const sortedIncomeCategories = budgetSession?.incomeCategories
    ? [...budgetSession.incomeCategories].sort((a, b) => {
        if (a.id === priorityCategoryId) return -1;
        if (b.id === priorityCategoryId) return 1;
        return 0;
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        {budgetSession?.incomeCategories &&
          budgetSession.incomeCategories.length > 0 && (
            <LayeredTreemaps
