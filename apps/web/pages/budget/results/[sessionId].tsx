...
export default function BudgetResultsPage() {
  ...
  const fetchResults = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/budget/results?sessionId=${sessionId}`,
      );
  ...
  const medianAllocations =
    result?.medianAllocations?.map((allocation) => ({
      ...
      subcategories:
        allocation.subcategories?.map((sub) => ({
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-800">
            These results represent the collective median budget. The median
            value was calculated for each category across all{" "}
            {result?.voterCount} proposals, then expense percentages were
            balanced to match the total median income. This ensures a fiscally
            responsible budget that reflects the community's priorities.
          </p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex items-center justify-between font-bold">
                ...
                <span>
                  {(result?.balancedExpenses / 1000000).toFixed(1)} mnkr
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex items-center justify-between font-bold">
                ...
                <span>
                  {(result?.totalMedianIncome / 1000000).toFixed(1)} mnkr
                </span>
              </div>
            </div>
          </div>
