...
export default function BudgetDebatePage() {
  ...
  useEffect(() => {
    ...
    const myUp = arguments_.find(
      (a) =>
        a.categoryId === selectedCategoryId && a.direction === "up" && a.isOwn,
    );
          ...
          prev.map((a) =>
            a._id === argumentId
              ? {
                  ...
                  userFoundHelpful: data.userFoundHelpful,
                }
              : a,
          ),
        );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-emerald-700 text-white p-5 shadow">
        <div className="max-w-5xl mx-auto">
          ...
          <h1 className="text-xl font-bold">Debatt — {budgetSession?.name}</h1>
  ...
  return (
    <div className={`border ${borderColor} rounded-xl overflow-hidden`}>
      ...
      <div className={`${headerBg} px-4 py-3 flex items-center gap-2`}>
        ...
        <span
          className={`ml-auto text-xs px-2 py-0.5 rounded-full ${badgeBg} ${badgeText}`}
        >
...
function ArgumentCard({ arg, onHelpful, onHide }) {
  return (
    <div
      className={`bg-white border rounded-lg p-3 ${arg.isOwn ? "border-blue-200 bg-blue-50/30" : "border-gray-200"}`}
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <span className="text-xs text-gray-400 shrink-0 ml-auto">
          {new Date(arg.createdAt).toLocaleDateString("sv-SE", {
            day: "numeric",
            month: "short",
          })}
        </span>
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          title={
            arg.isOwn
              ? "Du kan inte markera ditt eget argument"
              : "Bra argument"
          }
