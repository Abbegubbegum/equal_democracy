...
- `GET/PATCH /api/admin/proposals` — CSRF-protected; GET enriches each proposal with its session's `place` (`sessionPlace`); PATCH edits title/problem/solution/estimatedCost/status (no longer supports POST/create)
...
- `GET/PATCH /api/admin/citizen-proposals` — CSRF-protected; list all citizen proposals / update status (active/archived/selected/submitted_as_motion/rejected) and/or edit title/description
