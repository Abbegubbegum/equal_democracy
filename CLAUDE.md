...
**EAS production builds:** Package name `se.vallentunaframat.app`, owner `vallentuna-framat`. Production AAB is built and released to Google Play Console under internal testing. iOS TestFlight build is pending (requires Apple Developer account). Build command: `npx eas-cli build --profile production --platform android` from `apps/mobile` (`eas.json`'s `production` profile has `autoIncrement: true`, so `versionCode` bumps automatically each build). Android submission is automated via `eas submit --platform android --profile production` (add `--id <buildId>` to target a specific build, or `--latest`) — a Google Play service-account key (`eas-submit@vallentuna-framat.iam.gserviceaccount.com`) is already stored on EAS's servers (not in this repo), so no local credentials file is needed; `eas submit` releases straight to the `internal` track. `eas.json`'s `submit.production` is intentionally left as `{}` since the key lookup is automatic once it's configured on the EAS side.
