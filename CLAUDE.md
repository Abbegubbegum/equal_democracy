...
│   ├── swish.ts             # Swish membership payment client: getMembership / createMembershipPayment / buildSwishUrl / openSwishApp / watchPayment. The app builds the `swish://` app-switch URL itself (Linking.createURL) because the return scheme differs between Expo Go (`exp://`) and a standalone build — the server returns only the token
...
| `QuestionVote`          | Ja/Nej votes on questions                                             | questionId, userId, choice — unique per (question, user)                                                                                                                                                                                                                                                      |
...
**Pre-election voting-rights quota (Fas 0):** Ahead of BankID verification, every user may cast at most **5** first-time votes before the election (2026-09-13). The limit is a hardcoded constant `PRE_ELECTION_LIMIT = 5` in both `mobile/questions/vote.ts` and `mobile/questions/index.ts` — no live DB count needed. Enforced server-side in `POST /api/mobile/questions/vote` — only gates a _brand-new_ vote (no existing `QuestionVote` row for that `questionId`+`userId`); changing an existing vote is always free and doesn't consume a slot. `GET /api/mobile/questions` returns `{ used, limit }` so `vote.tsx` and Hem can show "Du har röstat i X av Y möjliga frågor" and surface the 403 inline (a dedicated `voteError` state — deliberately NOT the screens' full-page `fetchError`, since hitting the quota is an expected, frequent state, not a load failure). Future phases (not yet built): once BankID is connected, a verified Vallentuna resident bypasses the quota entirely; after the election, quota is replaced by the membership-based rules (1 vote/fullmäktige for paying members, 2/calendar year for non-member residents).
...
Single yes/no questions (mobile Hem/Rösta tabs, and questions spawned from municipal agenda items) live entirely outside `Session` now — see `Question`/`QuestionVote`/`QuestionComment`/`QuestionCommentRating` in the Database Schema section. Web-facing question management is admin-only (no public web voting page exists yet — `/api/recent` still links `"#"` for these).
...
- `GET /api/mobile/questions` — Returns `{ questions, quota }` (Bearer auth). Active first, then closed/archived newest-first; each entry `{ id, text, imageUrl, isActive, deadline, categories, voteCounts, createdAt, userVote }`. `quota` is `{ used, limit }` (5-vote pre-election cap)
- `POST /api/mobile/questions/vote` — Cast/update a ja/nej vote (`{ questionId, choice }`). A brand-new vote is gated by the question being active and the pre-election quota; changing an existing vote is always free
...
- `DELETE /api/account/delete` — GDPR right-to-erasure: deletes all user data (proposals, comments, ratings, votes — including the `Question` family, citizen proposals + blob images, budget votes/arguments, login codes) then deletes the User record. Requires active NextAuth session.
...
- **`turbo.json`** — Task pipeline: build depends on `^build`, dev is persistent + no cache; build task declares all Vercel env vars in `env[]` so they're available during CI builds. `$schema` is pinned to a versioned URL (`https://v2-10-11.turborepo.dev/schema.json`) by `@turbo/codemod update` — re-run that codemod when bumping turbo rather than editing the version by hand
...
- **React Compiler lint rules are warnings in `apps/web`, and absent in `apps/mobile`** — `eslint-plugin-react-hooks` 7.1 (pulled transitively by `eslint-config-next` 16.3) turns on `react-hooks/immutability` and `react-hooks/refs`, which error on long-standing patterns in ~8 web files (effects calling a function declared further down; refs read during render). `apps/web/eslint.config.mjs` demotes them — plus `set-state-in-effect` — to `"warn"`, because `next build` runs ESLint and would otherwise fail. Real cleanups worth doing as their own task; don't re-promote them to errors without fixing the code first.
