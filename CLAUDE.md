...
│       ├── index.tsx        # Hem — fixed blue hero with SVG >> logo (react-native-svg, tight-cropped viewBox, width matched to the "Framåt" slogan text via onLayout) + scrollable party info cards
│       ├── my-questions.tsx # Mina frågor — personal view: pill-tabs built from the user's saved interest areas (Settings), each showing the active "voting"-type question matching that area's categories, or "Ingen aktivitet just nu"
...
**Mina frågor screen (my-questions.tsx):** Replaces Sessioner in the bottom tab bar (same slot, between Hem and Rösta) as the personalized view. On focus, reads the user's saved interest-area keys from SecureStore (`STORAGE_INTERESTS`, same key `SettingsModal` writes) — defaults to `["budget"]` if never saved — and renders one horizontally-scrollable pill-tab per matching `INTEREST_AREAS` entry, in `INTEREST_AREAS` order. Has its own gear icon (top-right of the header, same `SettingsModal` as Hem/Info) with `onSaved={load}` so changing interests refreshes the pill-tabs immediately rather than waiting for the next focus event. Fetches two sources in parallel: `/api/mobile/sessions/voting` (voting-type, same data as Rösta, now including each session's `categories`) and `/api/mobile/sessions/active` (standard/survey-type, now also including `categories`). For the active pill, looks up `INTEREST_TO_CATEGORIES[key]` and checks voting sessions first — if an active voting session's `categories` overlaps, renders the same interactive `VotingQuestionCard` used on Rösta (full Ja/Nej/Avstår voting). If no voting match, falls back to standard/survey sessions and renders `SessionQuestionCard` (proposals list with star ratings — see below) for the first match. If neither matches, shows "Ingen aktivitet just nu". If the user has no saved interests at all, shows a prompt to set them via Info → kugghjulet. Content area is a `ScrollView` (not a fixed View) since a revived proposals list can be long.

**SessionQuestionCard (lib/SessionQuestionCard.tsx):** Renders an older "standard"/"survey" session (multiple user-submitted proposals, not a single Ja/Nej/Avstå choice) inside Mina frågor's card slot, alongside `VotingQuestionCard`. Self-contained — fetches its own proposals via `/api/mobile/sessions/{id}/proposals` and manages rating/submission state internally (unlike `VotingQuestionCard`, which is controlled by its parent), since per-proposal optimistic rating updates and the submit-modal flow are entirely local concerns. Reuses the same `ProposalCard` and `SubmitModal` building blocks as the preserved Sessioner screen (`app/sessions.tsx`) — shows "Lägg ditt svar" only when `session.phase === "phase1"`. Only invoked as a fallback when no voting-type question matches the active category.

**VotingQuestionCard (lib/VotingQuestionCard.tsx):** Shared component extracted from `vote.tsx` so the interactive Ja/Nej/Avstår card (date line, question, three radio alternatives with result bars once voted, vote button / "voted" badge) renders identically on both **Rösta** and **Mina frågor**. Takes `session`, `selected`, `onSelect`, `onVote`, `submitting` as props — callers own the vote-submission logic (POST to `/api/mobile/quick-vote`) and local `selected` state; the component is purely presentational plus the radio-selection callback.

**Rösta screen (vote.tsx):** TikTok-style full-screen vertical carousel of voting sessions with infinite loop (triple-array pattern, same as `sessions.tsx` and `proposals.tsx`). Each page owns its background image (plain `<Image>` as `absoluteFill` inside the page View — NOT a shared double-buffer). Sessions without an image show a solid dark blue (`#002d75`) base. Uses `containerH` from `onLayout` (not `Dimensions`) so `pagingEnabled` snaps correctly after the tab bar. Active session ("Dagens fråga") renders the shared `VotingQuestionCard` (see above); results are hidden before voting to prevent bandwagon effect and appear immediately after voting. Past sessions always show read-only results. Share button uses React Native `Share.share()`. Yellow "Föreslå en fråga" button pinned to bottom opens a bottom-sheet modal that POSTs to `/api/mobile/suggest-question`. Fetches from `/api/mobile/sessions/voting` (active first, then closed/archived newest-first).

**MAJ assistant (XAIModal.tsx):** Branded as "MAJ" in all user-facing text (rebranded from "XAI"; the file name, the `/api/mobile/xai` route, and the internal `role: "xai"` message-type identifier are unchanged — only display strings and the system prompt changed). A dark-blue circle with a sparkles icon floats at the top-left corner of every tab except Hem (rendered in `(app)/_layout.tsx` as an absolute overlay, z-index 100). Tapping it opens a bottom-sheet chat with the Claude API via `/api/mobile/xai`. Shows context-aware quick-action chips based on the current tab (different prompts for Hem, Mina frågor, Rösta, Förslag, Arkiv) plus two common actions (write a comment, submit a proposal). Has a "Anmäl MAJ" flag button (top-right of the sheet) that users can tap to report bad AI output. Uses `claude-haiku-4-5-20251001` with a 300-token limit for quick, concise replies. The floating button hides itself while the modal is open to avoid double-tap confusion.
...
**Settings modal (mobile):** `lib/SettingsModal.tsx` is a self-contained bottom-sheet that reads and writes interest preferences to SecureStore internally. Props: `visible`, `onClose`, `onSaved?`. The gear icon appears in the top-right of the blue hero on the **Hem** tab (`index.tsx`), on **Mina frågor** (`my-questions.tsx`), and as a floating button on the **Info** tab (rendered in `(app)/_layout.tsx` when `normPath === "/membership"`). Exports `STORAGE_INTERESTS` and `STORAGE_INTERESTS_ONLY`; `INTEREST_AREAS` and `INTEREST_TO_CATEGORIES` live in `packages/types/src/categories.ts` (shared with the web admin's "Mina frågor" Översikt — see below) and are imported directly from `@repo/types` by `SettingsModal.tsx`, `InterestsModal.tsx`, and `my-questions.tsx` rather than re-exported.

**Every `THEMATIC_CATEGORIES`/`GEOGRAPHIC_CATEGORIES` value needs an `INTEREST_TO_CATEGORIES` home:** A `Session.categories` value that isn't mapped from at least one `INTEREST_AREAS` key can never surface in **Mina frågor**, no matter how a session is tagged — there's no interest-area tab a user could select that would match it. `INTEREST_TO_CATEGORIES["miljo"] = ["Miljö & klimat", "Bostäder", "Trygghet & säkerhet"]` (interest area "Miljö och klimat") exists specifically to give those three otherwise-orphaned thematic categories a path in; check this mapping covers any new category added to `packages/types/src/categories.ts`. The same mapping now also drives the web admin's "Mina frågor" Översikt (`/admin/my-questions`), which filters municipal items, budget categories, and citizen proposals by the same `INTEREST_AREAS` pills.

**Onboarding and Settings now share one interest-picking flow:** `InterestsModal.tsx` (the automatic post-login onboarding prompt) used to let the user pick raw `GEOGRAPHIC_CATEGORIES`/`THEMATIC_CATEGORIES` values and post them straight to `/api/mobile/user/interests` without ever writing `STORAGE_INTERESTS` — so a user who only completed onboarding (never opened the gear icon) saw empty pill-tabs in **Mina frågor**, which only reads `STORAGE_INTERESTS`. Fixed by switching `InterestsModal.tsx` to the same `INTEREST_AREAS` chips as `SettingsModal.tsx`: same `"budget"`-is-always-on `toggle()` guard, same `setItem(STORAGE_INTERESTS, ...)` + `INTEREST_TO_CATEGORIES`-mapped POST to `/api/mobile/user/interests`. The two thematic/geographic chip groups are now derived from `INTEREST_AREAS` (split at the first entry with a `groupLabel`) instead of hand-listing the two raw category arrays, so both modals stay in sync automatically if `INTEREST_AREAS` changes.
...
- `GET /api/mobile/sessions/active` — Active sessions list with proposals per session (excludes "voting" and "municipal" types); each entry includes `categories` (consumed by **Mina frågor**'s fallback `SessionQuestionCard` match)
...
- `GET /api/mobile/sessions/voting` — All voting-type sessions as an array: active first (isActive: true), then closed/archived newest-first (isActive: false). Each entry has `{ id, question, imageUrl, isActive, startDate, voteCounts, userVote, categories }`. Returns `[]` if none exist. `categories` powers the **Mina frågor** tab's per-interest-area matching.
...
| `/admin/my-questions`             | "Mina frågor" Översikt — municipal items + budget categories + citizen proposals flattened and filterable by `INTEREST_AREAS` pill (mirrors mobile's Mina frågor); image upload everywhere, full inline text/category edit for citizen proposals, links out to the dedicated sub-pages below for municipal/budget text edits (those need the full-array-safe PATCH/PUT, not a single-item overwrite) |
...
| `/manage-sessions`                | Session admin (non-super admins); also the target of the admin dashboard's "Rösta" tab                                                                                                                                                                                                                                                                                                               |
